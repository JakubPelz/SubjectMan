...
class SubjectAbl {
  ...
  async listByStudyProgramme(awid, dtoIn) {
    ...
    let foundSubjectsIdInStudyProgramme = [];

    // find subjects ids in study programme
    try {
      foundSubjectsIdInStudyProgramme = await this.studyProgrammeDao.get(dtoIn.studyProgrammeId);
    }
    catch (err) {
      if (err instanceof ObjectStoreError) {
    ...
    let foundSubjects = [];
    await foundSubjectsIdInStudyProgramme.subjects.forEach(async element => {

      // get all subjects from subjects
      try {
        const foundSubject = await this.dao.get(element.subjectId);
        foundSubjects.push(foundSubject);
        console.debug(foundSubjects);
      }
      catch (err) {
        if (err instanceof ObjectStoreError) {
          throw new Errors.ListByStudyProgramme.SubjectListFailed({ uuAppErrorMap }, err);
